...
import { Form, Input, Button, Layout, Typography, Modal, Spin, Divider, Alert } from 'antd';
...
const AdminAccount = () => {
  ...
  const handleUpdate = async (values) => {
    ...
    const { firstName, lastName, username, newPassword, password } = values;
    const result = await updateAccount(firstName, lastName, username, newPassword, password);
    if(!result.success) {
  ...
  const handleEmailUpdate = async (values) => {
    ...
    if(!result.success) {
      ...
      else emailForm.setFields([{ name: 'email', errors: [result.error] }, { name: 'password', errors: [result.error] }]);
  ...
  return (
    <Layout style={{ padding: '0 20px', background: 'none' }}>
      {loading ? (
        <Spin delay={200} />
      ) : error ? (
        ...
        <>
          ...
          <Layout style={{ background: 'none', display: 'flex', justifyContent: 'space-between', gap: '24px', flexDirection: 'row', marginTop: '48px', flexWrap: 'wrap', maxWidth: '400px' }}>
